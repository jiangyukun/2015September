function SearchBox($container) {
    ...
    this._itemTemplate = _.template($('#template1').text());
    this.init();
}
...
SearchBox.prototype = {
    ...
    addSearchItem: function (item) {
        ...
        var htmlStr = this._itemTemplate({id: id, text: text});
    ...
    deleteSearchItem: function (id) {
        var i, j, item, listener, self = this;
        for (i = 0; i < this.itemList.length; i++) {
            ...
            if (item.id == id) {
                ...
                self.$container.find('span[data-id=' + id + ']').parent().remove();
                item.remove();
