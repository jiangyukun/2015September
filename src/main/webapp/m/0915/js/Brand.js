...
Brand.prototype = {
    ...
    init: function () {
        var i, self = this;
        self.brandItemList.push(new BrandItem(this.$brandItemAll.attr('id'), this.$brandItemAll.text(), 'all', self));
    ...
    refreshBrand: function (brandItems) {
        ...
        for (i = 0; i < brandItems.length; i++) {
            ...
            if (row > 3) {
                ...
                var brandItem = new BrandItem(item.id, item.text, 'item', this);
